...
namespace cobalt {
  ...
  struct function {
    ...
    std::unordered_set<std::vector<types::type_base const*>, param_hash, param_eq> merge(function const& other) const {
  ...
  struct varmap;
  using symbol_type = std::variant<variable, std::shared_ptr<function>, types::type_base const*, varmap*>;
    ...
    std::unordered_set<sstring::string> include(varmap* other) const {
      ...
      if (!imported(other)) {
        for (auto [name, sym] : other->symbols) {
          ...
          if (!succ) {
            if (sym.index() == 2) it->second.merge(*std::get<2>(sym));
